...
include 'send_email.php'; // Include the email functionality
...
if ($stmt->execute()) {
    $success = true;
    $booking_id = $conn->insert_id; // Get the ID of the newly created booking
    $operator_name = '';
    ...
    if (!empty($email) && filter_var($email, FILTER_VALIDATE_EMAIL)) {
        try {
            $email_sent = sendBookingConfirmationEmail($booking_data);
            logEmailAttempt($email, $email_sent, $booking_id);
        } catch (Exception $e) {
            // Log the error but don't fail the booking
            error_log("Email sending failed: " . $e->getMessage());
            logEmailAttempt($email, false, $booking_id);
        }
    }
} else {
    $success = false;
    ...
    <style>
        * {
        ...
        .email-status {
            ...
            padding: 1rem;
            margin: 1rem 0;
            border-left: 4px solid <?php echo ($success && $email_sent) ? '#4ade80' : '#fbbf24'; ?>;
        }
...
<body>
            ...
            <?php if (!empty($email)): ?>
                <?php if ($email_sent): ?>
                    <div class="email-status success">
                        ...
                        Controlla la tua casella di posta per i dettagli della prenotazione.
                    </div>
                <?php else: ?>
                    <div class="email-status warning">
                        <i class="fas fa-exclamation-triangle"></i>
                        <strong>Attenzione:</strong> Non è stato possibile inviare l'email di conferma.<br>
                        La prenotazione è comunque valida. Salva i dettagli qui sotto.
                    </div>
                <?php endif; ?>
            <?php else: ?>
                <div class="email-status warning">
                    <i class="fas fa-info-circle"></i>
                    <strong>Nessuna email fornita.</strong><br>
                    Salva i dettagli della prenotazione qui sotto.
                </div>
            <?php endif; ?>
    ...
    <script>
        // Auto redirect after 15 seconds for success
        <?php if ($success): ?>
        setTimeout(() => {
